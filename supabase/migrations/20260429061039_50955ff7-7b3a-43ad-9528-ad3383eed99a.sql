-- Drop existing schedule if it exists, then add fresh
DO $$
BEGIN
  PERFORM cron.unschedule('status-check-every-2min');
EXCEPTION WHEN OTHERS THEN
  NULL;
END $$;

SELECT cron.schedule(
  'status-check-every-2min',
  '*/2 * * * *',
  $cron$
  SELECT net.http_post(
    url := 'https://keagskdlvfjyegxqzrdv.supabase.co/functions/v1/status-check',
